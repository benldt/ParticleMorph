...
class _PMorpherState extends State<_PMorpher> with TickerProviderStateMixin{
  ...
  late List<vm.Vector3> _savedPositions;      // snapshot at touch start
  late List<double> _savedEffects;            // snapshot effects
  
  late AnimationController _returnCtl;        // return animation
  late Animation<double> _returnAnim;         // curved animation
  
  // Track morph state at touch
  ...
  Future<void> _boot()async{
    ...
    try{
      ...
      _savedPositions = List.generate(_Cfg.particleCount, (i) => vm.Vector3.zero());
      _savedEffects = List.filled(_Cfg.particleCount, 0.0);
  ...
  void _animate(){
    ...
    if (_touchActive) {
      _stepAttract(dt);
    } else if (_returnCtl.isAnimating) {
      _stepReturn();
    } else {
  ...
  void _onTouchStart(Offset position) {
    ...
    setState(() {
      ...
      final morphState = context.read<_MorphState>();
      if (morphState.busy) {
        _wasInMorph = true;
        _savedMorphProgress = morphState.t;
        _mCtl.stop(); // Pause morph
      } else {
        _wasInMorph = false;
      }
  ...
  void _onTouchEnd() {
    ...
    setState(() {
      ...
      _returnCtl.forward(from: 0);
      
      // Resume morph if it was interrupted
      if (_wasInMorph) {
        Timer(const Duration(milliseconds: 50), () {
          if (mounted && !_touchActive) {
            _mCtl.forward(from: _savedMorphProgress);
          }
        });
      }
    });
  ...
  void _stepAttract(double dt) {
    // Convert touch position to world coordinates using the same projection as the painter
    final size = MediaQuery.of(context).size;
    final center = Offset(size.width / 2, size.height / 2);
    final scale = math.min(size.width, size.height) / 400;
    
    // Unproject the touch position: reverse the painter's _project3D calculation
    // Touch is at screen coordinates (_touchPos.dx, _touchPos.dy)
    // We need to find the 3D world point that would project to this screen position
    
    // Assume the touch is on a plane at z = 0 in rotated space
    final screenX = (_touchPos.dx - center.dx) / scale;
    final screenY = -(_touchPos.dy - center.dy) / scale;
    
    // Reverse the perspective projection (assume depth of 300 + 0 = 300)
    ...
    final touchWorld = vm.Vector3(
      worldX * cosY - 0 * sinY,  // z=0 in rotated space
      worldY,
      worldX * sinY + 0 * cosY,
    );
    ...
    for (int i = 0; i < _particles.length; i++) {
      ...
      if (distance > 30.0) {  // Larger orbital radius
        ...
        // Add orbital motion for visual interest
  ...
  void _stepReturn() {
    final t = _returnAnim.value;
    final easeT = Curves.easeOutBack.transform(t);
    
    for (int i = 0; i < _particles.length; i++) {
      // Smooth interpolation with overshoot
      _particles[i] = _particles[i] * (1 - easeT) + _savedPositions[i] * easeT;
      _effects[i] = _effects[i] * (1 - t) + _savedEffects[i] * t;
    }
    
    // Apply idle breathing to saved positions
    if (!_wasInMorph) {
      final scale = 1 + math.sin(_elapsed * 0.5) * 0.015;
      final currentShape = context.read<_MorphState>().shape;
      for (int i = 0; i < _particles.length; i++) {
        final base = _shapes[currentShape][i];
        _savedPositions[i] = base * scale;
      }
    }
  }
  ...
  void _autoMorph(){
    ...
    if (_touchActive || _returnCtl.isAnimating || _mCtl.isAnimating) return;
  ...
  @override Widget build(BuildContext ctx) {
    ...
    // Wrap with gesture detection if enabled
